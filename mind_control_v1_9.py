...
ROBOT ENFOCADO PARA EL PAR: USD/CHF u otros pares de divisas con la misma proporcion
...
from zeromq_mt4_cliente_v1_9 import zeromq_mt4_ea_client
from data_parser_v1_9 import parser,parser_data,parser_donchian
'''
while True:
    digits = trading("GET",2,simbolo,1,0)
    pip = (10*pow(10,-digits))/10
    bidask = parser_data(zeromq_mt4_ea_client("RATES|{}".format(simbolo)))
    while(len(bidask) < 2):
        bidask = parser_data(zeromq_mt4_ea_client("RATES|{}".format(simbolo)))    
    precio = round(((float(bidask[0])+float(bidask[1]))/2)-1*pip,digits)    
    print("Precio: ",precio)
    time.sleep(0.1)
'''
# Funciones
...
def trading(accion,tipo,simbolo,stoploss,precio):
    ...
    if(accion=="OPEN"):
        ticket =zeromq_mt4_ea_client("TRADE|OPEN|{}|{}|{}|0|{}|50|Robot-Python-v9.9".format(tipo,simbolo,lote,stoploss))
        return ticket
    if(accion=="MODIFICAR"):
...
def get_indicador(tipo="TODOS",periodos=14):
    ...
    if(tipo == "TODOS"):
        ...
        while(len(indicadores) < 9 or indicadores[0] == simbolo):
            indicadores = parser_data(zeromq_mt4_ea_client("INDICADOR|{}|0|{}|{}".format(simbolo,periodos,tipo)))
            try:
        ...
        if(donchian_inf >= precio):    # Indica que va haber una tendencia Alcista
            ...
            while(cont < cont_tempo_seg):
                ...
                if(emarapida_m5 >= emalenta_m5_actual and rsi_actual > rsi_pasado and rsi_actual > 50 
                   ...
                   and abs(emalenta_m5_actual-emalenta_m5_pasado) > 25*pip):
                    flag_condiciones = 1
                    break
                cont += 0.38
                time.sleep(0.38)
