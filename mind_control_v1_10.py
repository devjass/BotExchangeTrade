...
ROBOT ENFOCADO PARA EL PAR: USD/JPY u otros pares de divisas con la misma proporcion
a nivel precio, es decir que el precio no supere el valor de 2.0

Tiene los mismos temporizadores que la version 5.3.
...
from zeromq_mt4_cliente_v1_10 import zeromq_mt4_ea_client
from data_parser_v1_10 import parser,parser_data,parser_donchian
'''
while True:
...
def trading(accion,tipo,simbolo,stoploss,precio):
    ...
    if(accion=="OPEN"):
        ticket =zeromq_mt4_ea_client("TRADE|OPEN|{}|{}|{}|0|{}|50|Robot-Python-v9.10".format(tipo,simbolo,lote,stoploss))
        return ticket
    if(accion=="MODIFICAR"):
    ...
    if(accion=="GET" and tipo == 1):
        ...
        while( len(data) < 6):
            data = parser_data(zeromq_mt4_ea_client("TRADE|GET|{}|1".format(ticket)))
            try:
                while(float(data[4]) > 120 or float(data[4]) < 70 ):
                    data = parser_data(zeromq_mt4_ea_client("TRADE|GET|{}|1".format(ticket)))
            except:
                data = parser_data(zeromq_mt4_ea_client("TRADE|GET|{}|1".format(ticket)))
            else:
                ...
                pass
            try:
                if(float(data[4]) < 120 and float(data[4]) > 70 ):
                    break
            except:
                pass
            finally:
    ...
    if(accion=="GET" and tipo == 3):
        ...
        while(len(data) < 2):
            data = parser_data(zeromq_mt4_ea_client("TRADE|GET|123456|3"))
            try:
                while(float(data[0]) > 120 or float(data[0]) < 70 ):
                    data = parser_data(zeromq_mt4_ea_client("TRADE|GET|123456|3"))
            except:
                data = parser_data(zeromq_mt4_ea_client("TRADE|GET|123456|3"))
            else:
                ...
                pass
            try:
                if(float(data[0]) < 120 and float(data[0]) > 70 ):
                    break
            except:
                pass
            finally:
        ...
        if(donchian_inf >= precio):    # Indica que va haber una tendencia Alcista
            ...
            while(cont < cont_tempo_seg):
                ...
                if(emarapida_m5 >= emalenta_m5_actual and rsi_actual > rsi_pasado and rsi_actual > 50 
                   ...
                   and abs(emalenta_m5_actual-emalenta_m5_pasado) >= 1*pip
