...
from zeromq_mt4_cliente_v1_4 import zeromq_mt4_ea_client
...
def get_ganancia():
    ...
    hoja=doc.get_sheet_by_name("TE USD-JPY")                #El usuario debe seleccionar y escribir el nombre de la hoja a escanear, se guarda en la variable hoja
...
if(hora < 25 and hora >= 12):
    x = 24-hora+7
    minutos = minutos + 30
elif(dtn.hour > 0 and dtn.hour < 7):
    x = 7-hora
    minutos = minutos + 30
elif(dtn.hour < 12 and dtn.hour >= 7):
    x = 0
    if(dtn.hour == 7):
        if(dtn.minute >= 0 and dtn.minute <30 ):
            minutos = 30 - dtn.minute
        elif(dtn.minute >= 30):
            minutos = 0
    else:
        minutos = 0
...
# El siguiente código es para guardar el listado de ordenes en un documento excel
